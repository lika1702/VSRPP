...
// SnopokLR_5Set.h: интерфейс класса CSnopokLR_5Set
//


#pragma once

// создаваемый код 29 октября 2021 г., 18:23

class CSnopokLR_5SetAccessor
{
public:
	LONG m_index;
	TCHAR m_login[256];
...
// потребуется сохранить пароль в другом виде или использовать другую аутентификацию.
	...
	BEGIN_COLUMN_MAP(CSnopokLR_5SetAccessor)
...
class CSnopokLR_5Set : public CCommand<CAccessor<CSnopokLR_5SetAccessor>> {
public:
	...
	{
		HRESULT hr = Open(m_session, L"select * from [User]", pPropSet);
#ifdef _DEBUG
		if(FAILED(hr))
			AtlTraceErrorRecords(hr);
#endif
	...
	HRESULT  AddRow(CString login, CString password, CString day, CString month, CString year, CString hour, CString minute, CString duration)
	{
		CDBPropSet propset(DBPROPSET_ROWSET);
		propset.AddProperty(DBPROP_IRowsetLocate, true);
		GetRowsetProperties(&propset);
		CString execute_str;
		execute_str.Format(L"INSERT INTO [User] ([Логин], [Пароль], [День], [Месяц], [Год], [Часы], [Минуты], [Длительность сессии]) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')", login, password, day, month, year, hour, minute, duration);
		Close();
		HRESULT hr = Open(m_session, execute_str, &propset, 0, DBGUID_DEFAULT, false);
		Close();
		hr = Open(m_session, L"select * from student order by ID", &propset);
