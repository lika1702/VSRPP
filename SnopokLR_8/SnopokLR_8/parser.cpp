...
void TParser::SendError(int errNum)
{
   ...
   if(*curToken=='\0')
      strcpy_s(curToken, "EOL");

   switch(errNum)
   {
      case 0:
         sprintf_s(buffer, "Unknown keyword: '%s'", curToken);
         errs[0] = buffer;
         break;

      case 1:
         sprintf_s(buffer, "Unknown symbol: '%s'", curToken);
         errs[1] = buffer;
         break;

      case 6:
         sprintf_s(buffer, "Unexpected '%s'", curToken);
         errs[6] = buffer;
         break;
   }
